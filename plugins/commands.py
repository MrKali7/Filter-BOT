...
async def start(client, message):
    ...
    if data.split("-", 1)[0] == "Ref":
        ...
        if vj and PREMIUM_AND_REFERAL_MODE:
            ...
            # Check if the user has reached the referral milestone
            if num_referrals % REFERAL_COUNT == 0:  # Incrementally add premium time
                time = REFERAL_PREMEIUM_TIME       
                seconds = await get_seconds(time)
                if seconds > 0:
                    # Fetch current expiry time or set it to now if not available
                    existing_user = await db.get_user(user_id)
                    current_expiry_time = existing_user.get("expiry_time") if existing_user else datetime.datetime.now()

                    # Add the new premium time
                    new_expiry_time = current_expiry_time + datetime.timedelta(seconds=seconds)
                    ...
                    await client.send_message(
                        chat_id=user_id,
                        text=f"<b>You have successfully completed a referral milestone.\n\nYou are added to Premium for {REFERAL_PREMEIUM_TIME}</b>"
                    )
                return
        else:
            ...
            if not await db.has_premium_access(message.from_user.id):
                if not await check_verification(client, message.from_user.id) and VERIFY == True:
                    btn = [[
                        ...
                        InlineKeyboardButton('💳 Gᴇᴛ Fʀᴇᴇ Oʀ Pᴀɪᴅ Sᴜʙsᴄʀɪᴘᴛɪᴏɴ 💳', callback_data='GetRefral')
                    ]]
                    await message.reply_text(
    ...
    if not files_:
        ...
        try:
            ...
            title = '@GoBoXMoviez  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
